...
using System.Data.Common;
using System.Data.Entity.Infrastructure;
using Action = My.Template.Model.Action;
...
namespace My.Template.EFDAL
{
    ...
    public partial class UserDal : BaseDal<User>, IUserDal
    {
        ...
        {
            List<Model.Action> user_actions = 
                          (from a in db.Set<Model.Action>()
                          join ua in db.Set<User_Action>() 
                          on a.ID equals ua.ActionID
                          where ua.UserID == user.ID
                          select a).ToList();
            List<Model.Action> rolesactions = (from a in db.Set<Model.Action>()
                               join b in db.Set<Role_Action>()
                                   on a.ID equals b.ActionID
                               join c in db.Set<Role>()
                                   on b.RoleID equals c.ID
                               join d in db.Set<User_Role>()
                ...
                foreach (var rolesaction in rolesactions)
                {
                    if (!user_actions.Contains(rolesaction))
                    {
                        user_actions.Add(rolesaction);
                    }
                }
        ...
        {
            //没有冻结的角色
            List<Model.Role> user_roles =
                          (from a in db.Set<Model.Role>()
                           join ua in db.Set<User_Role>()
                           on a.ID equals ua.RoleID
                           where ua.UserID == user.ID
                           where a.IsFrozen == false
