...
; (function ($, undefined) {
    ...
    function finishStep(wizard, state) {
        ...
        //将 图文消息 组织json数据 放入 form表单的隐藏域 然后提交表单。。。
        var samplehouseid = $("#houseID").val();
        var  areaStylePriceAndPics = { 'samplehouseid': samplehouseid, 'styleandpics': [] };
        //[ {samplehouseid:0,stysamplehouseidleandpics:[{areaid:1,styleid:2,price:1380,pics:'jhjhjh'},{areaid:1,styleid:2,price:1380,pics:'jhjhjh'}]},{areaid:0,styleandpics:[{areaid:1,styleid:2,price:1380,pics:'jhjhjh'},{areaid:1,styleid:2,price:1380,pics:'jhjhjh'}]}]

        var areaAndstyepics = $("#choseProLablesDiv").children(".onelable");
        var index = 0;
        for (var i = 0; i < areaAndstyepics.length; i++) {
            //获得选中的风格
            var lis = $(areaAndstyepics[i]).children(".arearight").children(".rightUL").children("li");
            for (var j = 0; j < lis.length; j++) {
                var areaid = $(lis[j]).children(".styleleft").find(".btn-deletstyle").data("areaid");
                var styid = $(lis[j]).children(".styleleft").find(".btn-deletstyle").data("styid");
                var styprice = $(lis[j]).children(".styleleft").children("span").find(".price4original").val();
                var styprice4discount = $(lis[j]).children(".styleleft").children("span").find(".price4discount").val();
                var buypersonnum = $(lis[j]).children(".styleleft").children("span").find(".buypersonnum").val();
                var endtime = $(lis[j]).children(".styleleft").children("span").find(".endtime").val();
                var designerID = $(lis[j]).children(".styleleft").children("span").find(".desgnerinfos").find("option:selected").val();
                var price4sequence = $(lis[j]).children(".styleleft").children("span").find(".price4sequence").val();
                var pics = $(lis[j]).children(".rightPics").children(".wrap_uploadImg2").find("input.hidesamplepics").val();

                var ardetails = "";
                for (var k = 0; k < eds.length; k++) {
                    if (eds[k].id === "areadetail_" + areaid + styid) {
                        ardetails = eds[k].obj.html();
                    }
                }
               
                areaStylePriceAndPics.styleandpics[index] = { 'areaid': areaid, 'styleid': styid, 'price': styprice, 'price4discount': styprice4discount, 'price4sequence': price4sequence, 'pics': pics, 'designerID': designerID, 'ardetails': ardetails, 'buypersonnum': buypersonnum, 'endtime': endtime };
                index++;
            }
        }

        index = 0;
        $("#areaStylePriceAndPics").val(JSON.stringify(areaStylePriceAndPics));
