...
function finishStep(wizard, state) {
    ...
    var picAndContent=[];
    var formlist = $(".addPicAndContentForm");
    for (var i = 0; i < formlist.length; i++) {
        var picval = $(formlist[i]).children(".form-group").children(".col-lg-8").children(".wrap_uploadImg1").children(".upload_container").children("input").val();
        var contentval = $(formlist[i]).children(".sharecontentDiv").children(".sharecontent").val();
        var hideID = $(formlist[i]).children(".hideID").val();
        picAndContent[i] = { ID: hideID, Pic: picval, Content: contentval };
    }

    $("#SingleSharePicAndContentStr").val(JSON.stringify(picAndContent));
